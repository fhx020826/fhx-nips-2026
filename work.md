...
3. 已将“每批粗读都要同步核查 shortlist”的规则写入：
   - `user.md`
4. 已对 shortlist 中与前 `10` 篇相关的条目完成复核，并修正以下描述：
...
2. 现在最重要的不是扩展更多杂项脚本，而是继续稳步完成 `61` 篇论文的高质量粗读。
3. 当前最值得后续继续关注的方向仍然是：
...
1. 继续生成第 `11` 到第 `15` 篇详细粗读卡片。
2. 每完成一批，同步复核 `docs/paper_quality_shortlist.md`。
3. 在进入任何 Python 运行任务前，先解决 conda 环境可用性问题。
4. 后续视需要继续整理：
