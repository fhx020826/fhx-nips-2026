...
2. 模板已从“自由长文式粗读”升级为“叙述 + 结构化字段”双层格式，新增：
   ...
   - `是否值得继续投入`
   - `核心问题映射`
3. 新模板已明确后续 5 篇一批的推荐粗读流程，方便继续半自动化 / 批量化推进。
...
10. 虽然统一模板已经落地，但尚未用新模板回填现有 Batch 1 文档，当前旧卡片和新模板之间还存在字段层不完全对齐。
...
5. 第二批优先补：
   ...
   - `DREAMWALKER`
   - `Open-Nav`
6. 先用新模板回填或重构后续粗读批次，避免继续沿用旧卡片格式。
7. 在第二批粗读之后，单独整理一页 `baseline / codebase reconnaissance` 文档，聚焦：
   - 仓库是否可跑
   - 依赖栈是否过旧
   - 数据 / 权重是否可得
   - 最小可验证起点应选谁
