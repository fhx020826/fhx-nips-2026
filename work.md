...
1. 继续生成第 `26` 到第 `35` 篇详细粗读卡片。
2. 每完成一批，继续同步复核 `docs/paper_quality_shortlist.md`。
3. 在粗读推进同时，逐步准备高优先级论文的 codebase reconnaissance，当前优先顺序建议先看：
   - `Ground Slow, Move Fast`
   - `JanusVLN`
   - `ETP-R1`
   - `CLASH`
   - `StreamVLN`
4. 在进入任何 Python 运行型任务前，先解决 conda 环境可用性问题。
